    ...
    return (
        ...
        >
            <motion.div
                ...
                className="group relative h-full bg-white/40 backdrop-blur-md border border-white/60 rounded-[32px] p-6 shadow-[0_8px_32px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_60px_rgba(11,94,215,0.15)] transition-all duration-300 pointer-events-auto flex flex-col"
            >
                ...
                <div className="relative z-10 flex flex-col h-full transform-style-3d group-hover:translate-z-10">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <Link
                                ...
                                className="w-8 h-8 rounded-full bg-white border border-[#E6F0FF] flex items-center justify-center text-[#0B5ED7] hover:bg-[#0B5ED7] hover:text-white transition-colors shadow-sm"
                                title="Edit"
                            >
                    ...
                    <h3 className="font-bold text-xl text-[#0A1A2F] leading-tight mb-2 group-hover:text-[#0B5ED7] transition-colors line-clamp-2">
                        {event.title}
                    </h3>

                    <div className="text-sm text-[#64748B] mb-4 space-y-1">
                        <div className="flex items-center gap-2">
                            <svg className="w-4 h-4 text-[#0B5ED7]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                            <span>{formatDate(event.start_time)}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <svg className="w-4 h-4 text-[#0B5ED7]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            <span>{formatDate(event.end_time)}</span>
                        </div>
                    </div>

                    <p className="text-sm text-[#64748B] line-clamp-2 mb-4 flex-grow font-light">
                        {event.description || 'No description provided.'}
                    </p>

                    <div className="mt-auto pt-4 border-t border-[#E6F0FF] flex justify-between items-center">
                        <span className="text-[10px] font-mono text-[#94A3B8]">ID: {event.id}</span>
                        <Link
                            href={`/admin/events/${event.id}/edit`}
    ...
    if (loading) return (
        <div className="flex items-center justify-center h-[60vh]">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-[#64748B] font-medium animate-pulse">Loading Events...</p>
            </div>
        </div>
    );
                ...
                {events.length === 0 && (
                    ...
                    >
                        <div className="w-24 h-24 rounded-full bg-[#E8F1FF] flex items-center justify-center mb-6">
                            <svg className="w-10 h-10 text-[#0B5ED7]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                        </div>
                        <h3 className="text-2xl font-bold text-[#0A1A2F] mb-2">No Events Yet</h3>
                        <p className="text-[#64748B] max-w-md mx-auto mb-8">Start by creating your first event to showcase.</p>
                        <Link
                            href="/admin/events/create"
